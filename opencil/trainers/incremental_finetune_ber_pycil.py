...
class FinetuneBERLearner(BaseLearner):
        ...
        if self._cur_task == 0:
            ...
            for _, epoch in enumerate(prog_bar):
                ...
                for i, (train_datas, train_targets) in enumerate(train_loader):
                    ...
                    beta=torch.distributions.beta.Beta(self.config.trainer.beta, self.config.trainer.beta).sample([]).item()
                    index = torch.randperm(train_targets.size(0)).cuda()

                    train_attributions = self._network(train_datas)
                    aux_train_logits = train_attributions["aux_logits"]

                    dummy_datas = beta * train_datas + (1 - beta) * train_datas[index]
                    dummy_logits = self._network(dummy_datas)["aux_logits"]

                    loss_clf = F.cross_entropy(aux_train_logits, train_targets)
                    
                    Ec_in = -torch.logsumexp(aux_train_logits, dim=1)
                    Ec_out = -torch.logsumexp(dummy_logits, dim=1)             
                    loss_ood = 0.1*(torch.pow(F.relu(Ec_in-self.config.trainer.m_in), 2).mean() + (torch.pow(F.relu((self.config.trainer.m_out - Ec_out)), 2)).mean())

                    loss = loss_clf + loss_ood
                    ...
                    _, preds = torch.max(aux_train_logits, dim=1)
            ...
            for _, epoch in enumerate(prog_bar):
                ...
                for i, (train_datas, train_targets) in enumerate(train_loader):
                    ...
                    train_beta=torch.distributions.beta.Beta(self.config.trainer.beta, self.config.trainer.beta).sample([]).item()
                    index = torch.randperm(train_targets.size(0)).cuda()
                    val_beta = 0.002
                    ...
                    dummy_datas = train_beta * train_datas + (1 - train_beta) * train_datas[index]
                    dummy_train_logits = self._network(dummy_datas)["aux_logits"]
                    ...
                    loss_clf = F.cross_entropy(aux_train_logits, train_targets)
                    loss_clf += F.cross_entropy(mixed_val_logits, val_targets) 
                    
                    Ec_train_in = -torch.logsumexp(aux_train_logits, dim=1)
                    Ec_val_in = -torch.logsumexp(mixed_val_logits, dim=1)                       
                    loss_ood = 0.1 * torch.pow(F.relu(Ec_train_in-self.config.trainer.m_in), 2).mean() 
                    loss_ood += 0.1 * torch.pow(F.relu(Ec_val_in-self.config.trainer.m_in), 2).mean()

                    Ec_train_out = -torch.logsumexp(dummy_train_logits, dim=1) 
                    loss_ood += 0.1 * (torch.pow(F.relu((self.config.trainer.m_out - Ec_train_out)), 2)).mean()                                                     
