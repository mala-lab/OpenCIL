...
class ODINCILPostprocessor(BaseCILPostprocessor):
    def __init__(self, config):
        super().__init__(config)
        self.args = self.config.postprocessor.postprocessor_args
    ...
    def postprocess(self, net: nn.Module, data: Any):
        ...
        # Adding small perturbations to images
        tempInputs = torch.add(data.detach(), gradient, alpha=-self.noise)
        output = net(tempInputs)['logits']

        # Use prediction for the last task
        # output = torch.cat(outputs, dim=1)
        output = output / self.temperature
...
class ODINCILFinetunePostprocessor(BaseCILPostprocessor):
    def __init__(self, config):
        super().__init__(config)
        self.args = self.config.postprocessor.postprocessor_args
